        	...
        	while(have_posts()) {
        		...
        		$res_name = '';
        			foreach($all_category as $category) {
        				if($category->term_id == 13 || $category->term_id == 14 || $category->term_id == 15){
        					$res_name = $category->slug;
        				}
        			}
        			?>
        			<div class="col-3 col-lg-6 col-sm-12 product mix <?= $res_name  ?> " data-order="<?= CFS()->get('diiplom_price'); ?>">
          <?php the_post_thumbnail(''); ?>
          <h3><?php the_title(); ?></h3>
          <div><?= CFS()->get('diiplom_price'); ?> &#8381;</div>
          <a href="<?php the_permalink(); ?>" class="btn"><?= CFS()->get('diiplom_more'); ?></a>
        </div>
        			<?php
        	}
        }
